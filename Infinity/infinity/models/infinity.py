...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            # =================================================================
            # VAR-Scaling (: Top-k Soft Sampling)
            # =================================================================

            if enable_scaling and si == scaling_layer_idx:
                if self.use_bit_label:
                    tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
                    logits_reshaped = logits_BlV.reshape(tmp_bs, -1, 2)
                else:
                    logits_reshaped = logits_BlV
                    tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
                    
                idx_samples = sample_with_top_k_top_p_also_inplace_modifying_logits_(
                    logits_reshaped, rng=rng, 
                    top_k=top_k or self.top_k, 
                    top_p=top_p or self.top_p, 
                    num_samples=scaling_num_samples
                )

                x = idx_samples.permute(0, 2, 1).float() # [B, N, seq_len]
                if self.use_bit_label:
                    x = x.reshape(tmp_bs, scaling_num_samples, tmp_seq_len, -1) # [B, N, HW, 1]
                else:
                    x = x.unsqueeze(-1) # [B, N, seq_len, 1]

                    
                x = x.permute(0, 2, 1, 3).float() # [B, HW, N, C]
                batch_size, hw, n, c_dim = x.shape
                device = x.device
                ...
                for i in range(hw):
                    ...
                    h = (n * (c_dim + 2) / 4.) ** (-1. / (c_dim + 4)) * std_dev
                    bandwidth = h.mean() + 1e-6
                    kde = torch.exp(-dist_sq / (2 * bandwidth ** 2))
                    total_scores += kde.sum(dim=-1)
             ...
             # =============================================================
                ...
                probs = torch.linspace(k, 1, steps=k, device=device).float()
                probs = probs / probs.sum()

                probs_batch = probs.unsqueeze(0).expand(batch_size, -1)
                selected_rank_in_k = torch.multinomial(probs_batch, 1) # [B, 1]

                selected_n_indices = torch.gather(top_k_indices, 1, selected_rank_in_k).squeeze(1)

                batch_indices = torch.arange(batch_size, device=device)
                idx_Bld = idx_samples[batch_indices, :, selected_n_indices] # [B, seq_len]    
                
                if self.use_bit_label:
                    idx_Bld = idx_Bld.view(tmp_bs, tmp_seq_len, -1)
            else:
                if self.use_bit_label:
                    tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
                    logits_reshaped = logits_BlV.reshape(tmp_bs, -1, 2)
                    idx_Bld = sample_with_top_k_top_p_also_inplace_modifying_logits_(
                        logits_reshaped, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1
                    )[:, :, 0]
            ...
            if not self.use_bit_label and 'idx_Bld' not in locals():
                idx_Bld = idx_Bl
